...
COLUMN_ALIASES = {
    ...
    "bid_price_1": ["Bid", "Bid Price", "Best Bid", "Harga Bid"],
    "bid_volume_1": ["Bid Volume", "Bid Vol", "Best Bid Volume", "Volume Bid"],
    "ask_price_1": ["Offer", "Ask", "Offer Price", "Ask Price", "Best Offer", "Harga Offer"],
    "ask_volume_1": ["Offer Volume", "Offer Vol", "Ask Volume", "Best Offer Volume", "Volume Offer"],
}

OPTIONAL_COLUMNS = [
    "company_name", "open_price", "high_price", "low_price", "close_price", "volume", "value", "frequency",
    "foreign_sell", "foreign_buy", "bid_price_1", "bid_volume_1", "ask_price_1", "ask_volume_1",
]
NUMERIC_COLUMNS = [c for c in OPTIONAL_COLUMNS if c != "company_name"]
...
def _migrate_schema(conn: sqlite3.Connection) -> None:
    ...
    additions = {
        ...
        "frequency": "REAL", "foreign_sell": "REAL", "foreign_buy": "REAL", "bid_price_1": "REAL",
        "bid_volume_1": "REAL", "ask_price_1": "REAL", "ask_volume_1": "REAL", "raw_data": "TEXT",
    }
...
def init_database() -> None:
    ...
    with sqlite3.connect(DATABASE_FILE) as conn:
        conn.execute("""CREATE TABLE IF NOT EXISTS stock_daily (
            ...
            bid_price_1 REAL, bid_volume_1 REAL, ask_price_1 REAL, ask_volume_1 REAL,
            raw_data TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP, UNIQUE(trade_date, stock_code))""")
        # Kept for future realtime/API snapshots; the current BEI file does not require a second upload.
        conn.execute("""CREATE TABLE IF NOT EXISTS orderbook_snapshot (
            ...
            stock_code TEXT NOT NULL, bid_price_1 REAL, bid_volume_1 REAL, ask_price_1 REAL, ask_volume_1 REAL,
            raw_data TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP, UNIQUE(snapshot_date, snapshot_time, stock_code))""")
...
def save_dataframe(df: pd.DataFrame) -> int:
    ...
    if data.empty:
        return 0
    init_database()
    data = data.dropna(subset=["trade_date", "stock_code"])
    columns = ["trade_date", "stock_code", "company_name", "open_price", "high_price", "low_price", "close_price", "volume", "value", "frequency", "foreign_sell", "foreign_buy", "bid_price_1", "bid_volume_1", "ask_price_1", "ask_volume_1"]
    saved = 0
    with sqlite3.connect(DATABASE_FILE) as conn:
        for _, row in data.iterrows():
            raw_data = json.dumps(row.to_dict(), default=str, ensure_ascii=False)
            values = tuple(_db_value(row.get(c)) for c in columns) + (raw_data,)
            conn.execute(f"""INSERT INTO stock_daily ({', '.join(columns)}, raw_data)
                VALUES ({', '.join('?' for _ in values)})
                ON CONFLICT(trade_date, stock_code) DO UPDATE SET
                {', '.join(f'{c}=excluded.{c}' for c in columns[2:])}, raw_data=excluded.raw_data""", values)
            saved += 1
        conn.commit()
    return saved
...
def load_data(stock_code: str | None = None, days: int | None = None) -> pd.DataFrame:
    ...
    clauses: list[str] = []
    if stock_code:
        clauses.append("stock_code = ?"); params.append(str(stock_code).upper())
    if days is not None:
        clauses.append("trade_date >= date((SELECT MAX(trade_date) FROM stock_daily), ?)"); params.append(f"-{int(days) - 1} days")
